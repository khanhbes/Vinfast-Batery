...
const TelemetryDashboard = lazy(() => import('./TelemetryDashboard.jsx'))
...
function AiLabTab() {
  ...
  const loadDataset = async () => {
    ...
    setError('')
    try {
      const res = await api.aiDataset(vehicleId)
      setDataset(res?.data?.records || [])
      setNormalized(null)
      setTrainResult(null)
      setTestResult(null)
      api.emitPortalSuccess(`Đã tải ${res?.data?.total || 0} bản ghi dataset AI`)
    ...
    <div className="portal-shell flex h-screen overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-4 lg:p-6 portal-content-in">
          {activeTab === 'dashboard' && (
            <Suspense fallback={<div className="text-sm text-[var(--color-text-dim)]">⏳ Đang tải Dashboard...</div>}>
              <TelemetryDashboard />
            </Suspense>
          )}
