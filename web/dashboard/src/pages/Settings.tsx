...
function SmartChargerVault() {
  ...
  return <section aria-labelledby="smart-charger-title" className="space-y-5">
    ...
    <div className="space-y-2"><h3 className="text-sm font-semibold">Stored profiles <span className="ml-1 text-muted-foreground">({profiles.length})</span></h3>{busy === 'load' ? <div className="flex items-center gap-2 py-4 text-sm text-muted-foreground" role="status"><Loader2 className="h-4 w-4 animate-spin" />Loading profiles...</div> : profiles.length === 0 ? <p className="rounded-xl border border-dashed p-5 text-sm text-muted-foreground">No Shelly profiles match the current account or vehicle filter.</p> : profiles.map(profile => <div key={profile.deviceId} className="flex flex-col gap-3 rounded-xl border border-border/60 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><ResponsiveText strategy="name" className="block font-medium">{profile.displayName || 'Shelly Charger'}</ResponsiveText><ResponsiveText strategy="monoId" className="block font-mono text-xs text-muted-foreground" alwaysTooltip>{profile.deviceId}</ResponsiveText><p className="mt-1 text-xs text-muted-foreground">{profile.vehicleId ? `Vehicle ${profile.vehicleId}` : 'No vehicle linked'} · revision {profile.revision ?? '—'}</p></div><div className="flex flex-wrap items-center gap-2"><Badge variant={profile.cloudVerified ? 'secondary' : 'outline'}>{profile.cloudVerified ? 'Cloud verified' : 'Cloud not verified'}</Badge><Badge variant={profile.noLoadTestVerified ? 'secondary' : 'outline'}>{profile.noLoadTestVerified ? 'No-load passed' : 'No-load pending'}</Badge><Button variant="outline" size="icon" aria-label={`Revoke ${profile.displayName || profile.deviceId}`} title="Revoke profile" disabled={busy !== null} onClick={() => void revoke(profile)}>{busy === `revoke:${profile.deviceId}` ? <Loader2 className="animate-spin" /> : <Trash2 />}</Button></div></div>)}</div>
  </section>;
}
