      ...
      <CardContent>
        {filteredLogs.length === 0 ? <div className="py-12 text-center"><Activity className="mx-auto mb-3 h-8 w-8 text-muted-foreground" /><p className="font-medium">{logs.length ? 'No matching events' : 'No administrative events yet'}</p><p className="mt-1 text-sm text-muted-foreground">{logs.length ? 'Change the filter or search term.' : 'Events appear after the backend records an administrative action.'}</p></div> : <div className="overflow-x-auto">
          <table className="w-full min-w-[860px] text-sm"><thead><tr className="border-b text-left text-muted-foreground"><th className="p-3 font-medium">Time</th><th className="p-3 font-medium">Action</th><th className="p-3 font-medium">Actor</th><th className="p-3 font-medium">Entity</th><th className="p-3 font-medium">Details</th></tr></thead>
            <tbody>{filteredLogs.map(log => { const category = actionKind(log.action); return <tr key={log.id} className="table-row-hover border-b border-border/50 align-top last:border-0 hover:bg-muted/30"><td className="whitespace-nowrap p-3 text-muted-foreground">{formatTimestamp(log.timestamp)}</td><td className="p-3"><Badge variant={category === 'critical' ? 'destructive' : category === 'model' ? 'default' : 'secondary'}>{log.action}</Badge></td><td className="p-3"><div className="flex gap-2"><User className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" /><div><p>{log.actorEmail || 'System'}</p><ResponsiveText strategy="monoId" className="block max-w-48 font-mono text-xs text-muted-foreground" alwaysTooltip>{log.actorUid}</ResponsiveText></div></div></td><td className="p-3"><p>{log.entity}</p><ResponsiveText strategy="monoId" className="block max-w-48 font-mono text-xs text-muted-foreground" alwaysTooltip>{log.entityId}</ResponsiveText></td><td className="max-w-md p-3 text-muted-foreground"><p className="line-clamp-3" title={detailsText(log.details)}>{detailsText(log.details)}</p></td></tr>; })}</tbody>
          </table>
        </div>}
      </CardContent>
    </Card>
