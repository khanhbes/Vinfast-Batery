...
import { PortalHttpError } from '@/lib/httpClient';
...
const mediaViewUrl = (entry: CatalogEntry, kind: 'twoD' | 'threeD') => {
  const views = entry.media?.views;
  const asset = views && typeof views === 'object' ? (views as Dictionary)[kind] : null;
  if (!asset || typeof asset !== 'object') return '';
  const media = asset as Dictionary;
  const url = media.thumbnailUrl || media.heroUrl || media.mediumUrl;
  return typeof url === 'string' ? url : '';
};

function Field({ label, value, onChange, type = 'text', required = false, error, fieldId }: { label: string; value: unknown; onChange: (value: string) => void; type?: string; required?: boolean; error?: string; fieldId?: string }) {
  return <label className="space-y-1.5 text-sm"><span className="font-medium">{label}{required && <span className="text-destructive"> *</span>}</span><Input id={fieldId} aria-invalid={Boolean(error)} aria-describedby={error ? `${fieldId}-error` : undefined} className={error ? 'border-destructive focus-visible:ring-destructive' : ''} type={type} value={value == null ? '' : String(value)} onChange={event => onChange(event.target.value)} />{error && <span id={`${fieldId}-error`} className="block text-xs text-destructive">{error}</span>}</label>;
}
...
function Editor({ initial, onClose, onSaved }: { initial: CatalogEntry; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const twoDFileRef = useRef<HTMLInputElement>(null);
  const threeDFileRef = useRef<HTMLInputElement>(null);
  ...
  const payload = () => {
    const { catalogId: _catalogId, revision: _revision, status: _status, selectable: _selectable, updatedAt: _updatedAt, createdAt: _createdAt, identityKey: _identityKey, publishedAt: _publishedAt, ...editable } = draft;
    return editable;
  };
  const showError = (error: unknown) => {
    const portal = error instanceof PortalHttpError ? error : null;
  ...
  const upload = async (file: File, kind: 'twoD' | 'threeD') => {
    if (!draft.catalogId) { toast.error('Save the draft before uploading media'); return; }
    setBusy('media');
    try {
      const sourceUrl = window.prompt(`${kind === 'twoD' ? '2D photo' : '3D render'} source URL`) || '';
      ...
      const result = await catalogUploadMedia(draft.catalogId, file, { kind, sourceUrl, rightsConfirmed, credit: '', licenseNote: 'Approved by administrator' });
      setDraft(current => ({ ...current, media: result.data })); toast.success(`${kind === 'twoD' ? '2D photo' : '3D render'} uploaded`);
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Image upload failed'); }
    finally { setBusy(''); }
  };
  const twoDUrl = mediaViewUrl(draft, 'twoD');
  const threeDUrl = mediaViewUrl(draft, 'threeD');
    ...
    <div className="min-h-full bg-slate-950 text-slate-100">
      ...
      <div className="space-y-9 p-5 sm:p-8">
        <section className="space-y-4"><h3 className="border-b border-slate-800 pb-3 font-semibold">Identity</h3><div className="grid gap-4 sm:grid-cols-2">
          <Field label="Brand" required fieldId="catalog-brandName" error={fieldErrors.brandName} value={draft.brandName} onChange={value => updateTop('brandName', value)} />
          <Field label="Brand ID" required fieldId="catalog-brandId" error={fieldErrors.brandId} value={draft.brandId} onChange={value => updateTop('brandId', value)} />
          <Field label="Model" required fieldId="catalog-model" error={fieldErrors.model} value={draft.model} onChange={value => updateTop('model', value)} />
          <Field label="Variant" required fieldId="catalog-variant" error={fieldErrors.variant} value={draft.variant} onChange={value => updateTop('variant', value)} />
          <Field label="Model year" required fieldId="catalog-modelYear" error={fieldErrors.modelYear} type="number" value={draft.modelYear} onChange={value => updateTop('modelYear', Number(value))} />
          <Field label="Market (ISO-2)" required fieldId="catalog-market" error={fieldErrors.market} value={draft.market} onChange={value => updateTop('market', value.toUpperCase())} />
        ...
        <section className="space-y-4"><h3 className="border-b border-slate-800 pb-3 font-semibold">Vietnamese & English content</h3>{['vi','en'].map(locale => <div key={locale} className="grid gap-4 sm:grid-cols-2"><Field label={`${locale.toUpperCase()} display name`} required fieldId={`catalog-localized-${locale}-displayName`} error={fieldErrors[`localized.${locale}.displayName`]} value={localizedAt(draft, locale, 'displayName')} onChange={value => updateLocale(locale, 'displayName', value)} /><Field label={`${locale.toUpperCase()} tagline`} value={localizedAt(draft, locale, 'tagline')} onChange={value => updateLocale(locale, 'tagline', value)} /><div className="sm:col-span-2"><label className="space-y-1.5 text-sm"><span className="font-medium">{locale.toUpperCase()} description</span><textarea className="min-h-24 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={String(localizedAt(draft, locale, 'description'))} onChange={event => updateLocale(locale, 'description', event.target.value)} /></label></div></div>)}</section>
        <section className="space-y-4"><h3 className="border-b border-slate-800 pb-3 font-semibold">Battery & app defaults</h3><div className="grid gap-4 sm:grid-cols-2">
          <Field label="Calculation capacity (Wh)" required fieldId="catalog-appDefaults-calculationCapacityWh" error={fieldErrors['appDefaults.calculationCapacityWh']} type="number" value={valueAt(draft,'appDefaults','calculationCapacityWh')} onChange={value => updateGroup('appDefaults','calculationCapacityWh',numberOrNull(value))} />
          <Field label="Default efficiency (km/%)" required fieldId="catalog-appDefaults-defaultEfficiencyKmPerPercent" error={fieldErrors['appDefaults.defaultEfficiencyKmPerPercent']} type="number" value={valueAt(draft,'appDefaults','defaultEfficiencyKmPerPercent')} onChange={value => updateGroup('appDefaults','defaultEfficiencyKmPerPercent',numberOrNull(value))} />
        ...
        <section className="space-y-4"><div className="flex items-center justify-between border-b border-slate-800 pb-3"><h3 className="font-semibold">Official evidence</h3><Button variant="outline" className="border-slate-700 bg-transparent text-slate-100 hover:bg-slate-900" onClick={() => updateTop('sources',[...(draft.sources || []),{url:'',publisher:draft.brandName || '',type:'manufacturer'}])}><Plus />Source</Button></div>
          ...
          <input ref={twoDFileRef} className="hidden" type="file" accept="image/jpeg,image/png,image/webp" onChange={event => { const file=event.target.files?.[0]; if(file) void upload(file, 'twoD'); event.currentTarget.value=''; }} />
          <input ref={threeDFileRef} className="hidden" type="file" accept="image/jpeg,image/png,image/webp" onChange={event => { const file=event.target.files?.[0]; if(file) void upload(file, 'threeD'); event.currentTarget.value=''; }} />
          <div className="grid gap-3 sm:grid-cols-2"><div className="border border-slate-800 bg-slate-900/40 p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-medium">2D photo</p><p className="text-xs text-slate-400">Default image in the mobile app.</p></div><Button variant="outline" size="sm" className="border-slate-700 bg-transparent text-slate-100 hover:bg-slate-900" disabled={!draft.catalogId || busy === 'media'} onClick={() => twoDFileRef.current?.click()}>{busy === 'media' ? <Loader2 className="animate-spin" /> : <ImagePlus />}Upload</Button></div>{twoDUrl && <img className="mt-3 aspect-[16/9] w-full object-cover" src={twoDUrl} alt="2D vehicle" />}</div><div className="border border-slate-800 bg-slate-900/40 p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-medium">3D render</p><p className="text-xs text-slate-400">Alternative render shared with the app.</p></div><Button variant="outline" size="sm" className="border-slate-700 bg-transparent text-slate-100 hover:bg-slate-900" disabled={!draft.catalogId || busy === 'media'} onClick={() => threeDFileRef.current?.click()}>{busy === 'media' ? <Loader2 className="animate-spin" /> : <ImagePlus />}Upload</Button></div>{threeDUrl && <img className="mt-3 aspect-[16/9] w-full object-cover" src={threeDUrl} alt="3D vehicle render" />}</div></div>
        </section>
      </div>
...
export default function VehicleCatalog() {
  const [entries,setEntries]=useState<CatalogEntry[]>([]); const [loading,setLoading]=useState(true); const [loaded,setLoaded]=useState(false); const [loadError,setLoadError]=useState(''); const [query,setQuery]=useState(''); const [status,setStatus]=useState('all'); const [type,setType]=useState('all'); const [brand,setBrand]=useState('all'); const [market,setMarket]=useState('all'); const [year,setYear]=useState('all'); const [quality,setQuality]=useState('all'); const [selected,setSelected]=useState<CatalogEntry|null>(null); const [editor,setEditor]=useState<CatalogEntry|null>(null); const [research,setResearch]=useState(false); const [history,setHistory]=useState<Dictionary[]>([]);
  const load=useCallback(async()=>{setLoading(true);setLoadError('');try{const response=await catalogList();setEntries(response.data||[]);setLoaded(true);}catch(error){const message=error instanceof Error?error.message:'Catalog could not be loaded';setLoadError(message);toast.error(message);}finally{setLoading(false);}},[]);
