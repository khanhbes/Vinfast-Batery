...
class TripTrackingService {
  ...
  Future<void> _clearPersistedState() async {
    ...
    await prefs.remove('trip_ownerUid');
    await prefs.remove('trip_sessionId');
  }
  ...
  Future<TripStartResult> _doStartTrip({
    ...
    try {
      ...
      if (_ownerUid != null) {
        await prefs.setString('trip_ownerUid', _ownerUid!);
      }
      if (_tripSessionId != null) {
        await prefs.setString('trip_sessionId', _tripSessionId!);
      }
    } catch (e) {
  ...
  Future<void> _syncTelemetryPoint(Position pos) async {
    ...
    try {
      await FirebaseFirestore.instance.collection('TelemetryPoints').add({
        'trip_id': _tripSessionId,
        ...
        'current_soc': _currentBattery,
        'payloadType': _payload.value,
        'distanceKm': double.parse(_totalDistance.toStringAsFixed(3)),
        'isDeleted': false,
    ...
    try {
      ...
      await firestore.runTransaction((transaction) async {
        ...
        transaction.set(tripRef, {
          ...
          'durationSec': trip.duration.inSeconds,
          'avgSpeedKmh': double.parse(trip.avgSpeed.toStringAsFixed(2)),
          'routePointCount': _routePoints.length,
          'aiFeatures': {
            'payloadFactor': _payload.factor,
            'defaultEfficiency': _defaultEfficiency,
            'startBattery': _startBattery,
            'endBattery': _currentBattery,
            'batteryConsumed': consumed,
            'distanceKm': double.parse(_totalDistance.toStringAsFixed(3)),
